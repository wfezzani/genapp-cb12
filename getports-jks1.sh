...
sudo podman cp -a /var/tmp/genapp-github/. GENAPP-${BUILD_NUMBER}:$WA_DST_FOLDER/source/
sudo podman cp /var/tmp/genapp-github/project.dat GENAPP-${BUILD_NUMBER}:$WA_DST_FOLDER
sudo podman cp /var/tmp/genapp-github/wa-script.sh GENAPP-${BUILD_NUMBER}:$WA_DST_FOLDER
